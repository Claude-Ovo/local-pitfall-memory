...
Write-Output '== codex review #3 regression tests'
python (Join-Path $PSScriptRoot 'test_review3.py')
if ($LASTEXITCODE -ne 0) { Write-Output 'REVIEW3 FAIL'; exit 1 }
...
# entry contract: --continue with an EMPTY, isolated models dir and the network disabled → exit 3 + one JSON line, zero network
$env:PITFALL_MODELS_DIR = Join-Path $env:TEMP "pitfall-models-$(Get-Random)"; $env:PITFALL_OFFLINE = '1'
$s = Step 'continue-offline' { & $Run --continue }
$rc = $LASTEXITCODE; if ($rc -ne 3 -or $s -notmatch '"state": "pending"') { Write-Output "expected exit 3 + pending (got $rc)"; exit 1 }
Remove-Item -Recurse -Force $env:PITFALL_MODELS_DIR -ErrorAction SilentlyContinue
Remove-Item Env:PITFALL_MODELS_DIR; Remove-Item Env:PITFALL_OFFLINE
# --continue against the real models dir: exit 0 when complete, 3 when pending (may use the network if incomplete)
& $Run --continue | Out-Null
$rc = $LASTEXITCODE; Write-Output "[continue] exit=$rc"; if ($rc -ne 0 -and $rc -ne 3) { exit 1 }
# entry contract: structured error + exit 1 on bad input / bad arguments
$s = Step 'badfile' { & $Run lookup --request-file (Join-Path $PSScriptRoot 'nope.json') --json }
if ($LASTEXITCODE -ne 1 -or $s -notmatch '"error": "request_unreadable"') { Write-Output 'expected structured exit 1 on missing request file'; exit 1 }
$s = Step 'badargs' { & $Run lookup }
if ($LASTEXITCODE -ne 1 -or $s -notmatch '"error": "bad_arguments"') { Write-Output 'expected structured exit 1 on bad arguments'; exit 1 }
$s = Step 'digestdir' { & $Run digest --out $env:TEMP --json }
if ($LASTEXITCODE -ne 1 -or $s -notmatch '"error": "output_unwritable"') { Write-Output 'expected structured exit 1 on directory --out'; exit 1 }
