...
HEAD, TAIL = 2.0, 2.5
CUTS = [
    (5.5, 14.0, None, 0.92, 0, 0, "第二次遇到同一个坑"),
    (56.5, 60.5, None, 0.92, 0, 0, "Qoder 自己跑命令，把完整报错交给踩坑库"),
    (72.5, 75.0, None, 0.92, 0, 0, "Skill local-pitfall-memory activated"),
    (93.5, 102.5, 0.11, 1.0, 0, 0, "exact · 可引用 —— 本机已验证的修复，本地命中 < 1 秒"),
    (147.0, 149.0, 0.70, 2.0, 1.0, 0, "照修复卡改一行"),
    (156.5, 162.0, 0.70, 2.0, 1.0, 0, "一次过"),
...
def main():
    ...
    for (si, so, crop, scale, tx, ty, caption) in CUTS:
        ...
        l_segments.append(tseg(cm, cursor, dur, lower_seg_t, -0.4, -0.40))
        cursor += dur
